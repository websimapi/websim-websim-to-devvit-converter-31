...
export class AssetAnalyzer {
    ...
    processJS(jsContent, filename = 'script.js') {
        ...
        // We replace the base URL with a marker that is CSP safe, but we primarily rely on AST rewriting below for dynamic avatars.
        const AVATAR_MARKER = 'https://www.redditstatic.com/avatars/avatar_default_02_FF4500.png?user=';
        code = code.replace(/https:\/\/images\.websim\.ai\/avatar\/|https:\/\/images\.websim\.com\/avatar\//g, AVATAR_MARKER);
        ...
        try {
            ...
            const handleAvatarSwap = (varPart, fullNode) => {
                // Check for member expression accessing 'username'
                // We want to replace `... + post.username` with `post.avatar_url`
                if (varPart.type === 'MemberExpression' && 
                    varPart.property.type === 'Identifier' && 
                    varPart.property.name === 'username') {
                    
                    const objectCode = code.slice(varPart.object.start, varPart.object.end);
                    const replacement = `(${objectCode}.avatar_url || "https://www.redditstatic.com/avatars/avatar_default_02_FF4500.png")`;
                    magic.overwrite(fullNode.start, fullNode.end, replacement);
                    hasChanges = true;
                    return true;
                }
                return false;
                ...
                TemplateLiteral: (node) => {
                    if (node.quasis.length === 2 && node.expressions.length === 1) {
                        ...
                        if (isWebSim || isSwapped) {
                            handleAvatarSwap(node.expressions[0], node);
                        }
                    }
                },
                BinaryExpression: (node) => {
                    // Handle concatenation: "https://.../avatar/" + user.username
                    if (node.operator !== '+') return;
                    
                    let stringPart, varPart;
                    if (node.left.type === 'Literal' && typeof node.left.value === 'string') {
                        stringPart = node.left.value;
                        varPart = node.right;
                    } else if (node.right.type === 'Literal' && typeof node.right.value === 'string') {
                        stringPart = node.right.value;
                        varPart = node.left;
                    }
                    
                    if (stringPart) {
                        const isWebSim = stringPart.includes('images.websim.ai/avatar/') || stringPart.includes('images.websim.com/avatar/');
                        const isSwapped = stringPart.includes('redditstatic.com/avatars/') && stringPart.includes('?user=');
                        
                        if (isWebSim || isSwapped) {
                            handleAvatarSwap(varPart, node);
                        }
                    }
                }
            });
